...
- **NodeCount**: A simple servlet that provides results count of a SQL-2 query. JCR does not provide a "count(*)" function. This servlet uses the Apache Jackrabbit OAK NodeIterator.getSize() method if the **Fast return size** is enabled (OSGI configuration with PID org.apache.jackrabbit.oak.query.QueryEngineSettingsService) or count each returned node otherwise (very slow but it works when needed!). **How to enable the Fast return size option** <img width="1753" alt="image" src="https://user-images.githubusercontent.com/18739184/164893767-23551c86-5077-4347-975c-5d54476303ec.png">**Example of a count request when the Fast return size option is enabled**<img width="985" alt="image" src="https://user-images.githubusercontent.com/18739184/164896798-70fd1abd-b310-44b6-a409-4d673bbd0a27.png">**Example of a count request when the Fast return size option is disabled**<img width="986" alt="image" src="https://user-images.githubusercontent.com/18739184/164893852-b2ea371c-83e9-48d1-be89-59b44ae93e46.png">  Remember that the NodeIterator.getSize() method counts exactly (just for the Jackrabbit OAK implementation) all the nodes because ACLs are not applied to the results as per the [the official documentation](https://jackrabbit.apache.org/oak/docs/query/query-engine.html).
